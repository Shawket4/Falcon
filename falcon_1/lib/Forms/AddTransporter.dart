          ...
          children: <Widget>[
            ...
            Center(
              ...
              child: FlatButton(
                onPressed: () async =>
                {
                  if (_nameController.text.isEmpty || _phoneControllers[0].text.isEmpty || _phoneNameControllers[0].text.isEmpty || _emailController.text.isEmpty || _passwordController.text.isEmpty)
                    {
                      showCupertinoDialog(
                        context: context,
                        builder: (context) =>
                  ...
                  else
                    {
                      ...
                      for (int i = 0; i <= _phoneCount; i++) {
                        phoneNumbers.add({
                          _phoneNameControllers[i].text: _phoneControllers[i].text,
                        }),
                      },
                      print(phoneNumbers),
                      await http.post(Uri.parse("$SERVER_IP/api/RegisterTransporter"), headers: {
                        ...
                        // print(value.body);
                        await http.post(Uri.parse("$SERVER_IP/api/RegisterUser"), headers: {
                          "Content-Type": "application/json",
                          "Cookie": "jwt=${widget.jwt}",
                        }, body: jsonEncode({
